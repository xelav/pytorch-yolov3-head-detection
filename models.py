...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        # select feature extractor part of model
        self.feature_extractor = nn.ModuleList()
        for i in range(len(self.module_list)):
            if not isinstance(list(self.module_list[i].children())[0], YOLOLayer):
                # print(list(model.module_list[i].children())[0])
                self.feature_extractor.append(self.module_list[i])
            else:
                break
    ...
    def predict(self, image, device='cuda', img_size=446, conf_thres=0.6, nms_thres=0.2, normalize=False):
        ...
        outputs = non_max_suppression(result, conf_thres=conf_thres, nms_thres=nms_thres)
